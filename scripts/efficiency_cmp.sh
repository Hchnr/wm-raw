#!/usr/bin/env bash
# Efficiency comparison: 3 experiments on single-node, 2 GPUs each
# All 3 experiments run in background (non-blocking) on different GPU pairs
# Requires 6 GPUs total (GPU 0-1, 2-3, 4-5)
#
# Usage: bash scripts/efficiency_cmp.sh
# Kill all: pkill -f "wm_raw.train.*--wandb-name exp[123]"

set -euo pipefail

CONFIG="configs/gpic_image_diffusion.yaml"
NPROC=2
MAX_STEPS=1000

echo "=== Launching 3 efficiency experiments (1000 steps each) ==="

# Exp1: Baseline (no compile, no fused)
echo "[Exp1] Baseline (GPU 0,1)"
CUDA_VISIBLE_DEVICES=0,1 torchrun --nproc_per_node=$NPROC --master_port=29500 -m wm_raw.train \
    --config $CONFIG \
    --max-steps $MAX_STEPS \
    --compile false \
    --compile-mode default \
    --adam-fused false \
    --output-dir outputs/exp1_baseline \
    --log-dir logs/exp1_baseline \
    --wandb-project wm-training \
    --wandb-name exp1-baseline \
    > logs/exp1_baseline.log 2>&1 &
PID1=$!

# Exp2: Compile
echo "[Exp2] Compile (GPU 2,3)"
CUDA_VISIBLE_DEVICES=2,3 torchrun --nproc_per_node=$NPROC --master_port=29501 -m wm_raw.train \
    --config $CONFIG \
    --max-steps $MAX_STEPS \
    --compile true \
    --compile-mode default \
    --adam-fused false \
    --output-dir outputs/exp2_compile \
    --log-dir logs/exp2_compile \
    --wandb-project wm-training \
    --wandb-name exp2-compile \
    > logs/exp2_compile.log 2>&1 &
PID2=$!

# Exp3: Compile + Fused AdamW
echo "[Exp3] Compile + Fused AdamW (GPU 4,5)"
CUDA_VISIBLE_DEVICES=4,5 torchrun --nproc_per_node=$NPROC --master_port=29502 -m wm_raw.train \
    --config $CONFIG \
    --max-steps $MAX_STEPS \
    --compile true \
    --compile-mode default \
    --adam-fused true \
    --output-dir outputs/exp3_compile_fused_adamw \
    --log-dir logs/exp3_compile_fused_adamw \
    --wandb-project wm-training \
    --wandb-name exp3-compile-fused-adamw \
    > logs/exp3_compile_fused_adamw.log 2>&1 &
PID3=$!

echo ""
echo "All experiments launched:"
echo "  Exp1 (baseline)             PID=$PID1 (GPU 0,1)"
echo "  Exp2 (compile)              PID=$PID2 (GPU 2,3)"
echo "  Exp3 (compile-fused-adamw)  PID=$PID3 (GPU 4,5)"
echo ""
echo "Monitor logs:"
echo "  tail -f logs/exp{1,2,3}_*.log"
echo ""
echo "Wait for all to finish:"
echo "  wait $PID1 $PID2 $PID3"
